...
export default function ProfileScreen() {
  ...
  const handleAddPhoto = async () => {
    ...
    try {
      ...
      console.log = (...args) => {
        if (
          !(typeof args[0] === "string" && args[0].includes("Image Picker"))
        ) {
    ...
    return isEditing ? (
      <View style={styles.fieldContainer}>
        ...
        <TextInput
          ...
          value={inputValue}
          onChangeText={(text) =>
            setProfile({ ...profile, [key]: text || null })
          }
      ...
      <View style={styles.fieldContainer}>
        <PoppinsText style={styles.label}>{label}</PoppinsText>
        <PoppinsText
          style={[styles.value, !stringValue && styles.placeholderText]}
        >
          {stringValue || "Not set"}
        </PoppinsText>
      </View>
          ...
          {profile.photos.length > 1 && (
            <View style={styles.paginationContainer}>
              <TouchableOpacity
                style={[
                  styles.navButton,
                  selectedPhotoIndex === 0 && styles.disabledNavButton,
                ]}
                onPress={() => {
                  if (selectedPhotoIndex > 0) {
                    setSelectedPhotoIndex(selectedPhotoIndex - 1);
      ...
      <ScrollView style={styles.scrollView}>
        ...
        <View style={styles.profileHeaderContainer}>
          <View style={styles.profileHeader}>
            <View style={styles.mainPhotoContainer}>
              {profile.photos && profile.photos.length > 0 ? (
                <TouchableOpacity
                  onPress={() =>
                    profile.photos.length > 0 && setIsModalVisible(true)
                  }
                  activeOpacity={0.8}
                >
                  <Image
          ...
          <View style={styles.headerIcons}>
            ...
            >
              <Ionicons name="settings-outline" size={24} color="#651B55" />
            </TouchableOpacity>
            <TouchableOpacity
            ...
            >
              {isEditing ? (
                <Ionicons name="checkmark" size={24} color="#651B55" />
              ) : (
                <Image
          ...
          {Array.from({ length: 4 }).map((_, index) => {
            ...
            if (index < profile.photos.length) {
              ...
              return (
                <View
                  key={index}
                  style={[styles.thumbnailContainer, styles.emptyThumbnail]}
                >
                  {isEditing && (
                    <TouchableOpacity
        ...
        <View style={styles.section}>
          ...
          {profile.orientation &&
            renderField("Orientation", profile.orientation, "orientation")}
            ...
            {isEditing ? (
              <View style={styles.interestsContainer}>
                {INTERESTS.map((interest) => (
                  <TouchableOpacity
                    ...
                    onPress={() =>
                      setProfile({ ...profile, interests: [interest] })
                    }
                  >
                    <PoppinsText
                      style={[
              ...
              <View style={styles.interestsContainer}>
                {profile.interests && profile.interests[0] ? (
                  <View
                    style={[styles.interestTag, { backgroundColor: "#f0f0f0" }]}
                  >
        ...
        {isEditing ? (
          <TouchableOpacity
            style={[
              styles.button,
              styles.saveButton,
              (isUpdating || isLoading) && styles.disabledButton,
            ]}
            onPress={handleSave}
